...
public class Game {
    ...
    public boolean isFinished() {
        ...
        if (board.getValidMoves(players[0]).size()==0 && board.getValidMoves(players[1]).size()==0){
            resultat=true;
        }
        return resultat;
    }
    ...
    private Player confirmOrUndoMove(Player player) {
        ...
        int movevaluesi ;
       do {
            System.out.println("Reculer d'un coup -> saisir 1, ou 0 sinon :");
            movevaluesi = scan.nextInt();
            if (movevaluesi==1){
                board.undoFromMemento(caretaker.getMemento());
                resultat=getOtherPlayer(player);
            } else {
                if (movevalues==1){
                    resultat=getOtherPlayer(player);
                } else {
           ...
           movevalues=movevaluesi;
        } while (caretaker.getSize()>0&&movevaluesi==1);
